...
def test_validation_rejects_malformed_review_override_row() -> None:
    ...
    with TemporaryDirectory() as tmp:
        ...
        (output_dir / "post_adoption_bbox_span_diagnosis.json").write_text(
            json.dumps(
                {
                    "active_policy": "v1_consecutive_lines",
                    "summary": {"total_bbox_span_cases": 0},
                    "diagnoses": [],
                }
            ),
            encoding="utf-8",
        )
        (output_dir / "post_adoption_remediation_plan.json").write_text(
            json.dumps(
                {
                    "active_policy": "v1_consecutive_lines",
        ...
        (output_dir / "visual_review_cases_report.json").write_text(
            json.dumps(
                {
                    "active_policy": "v1_consecutive_lines",
                    "review_only": True,
